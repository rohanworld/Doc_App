...
public class AllPatients extends AppCompatActivity {
    ...
    private void filterAppointmentsByInOrOut(String filterKeyword) {
        mAppointments.clear();
        adapter1.notifyDataSetChanged();
        db.collection("patients").whereEqualTo("inOrOut", filterKeyword).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                for(QueryDocumentSnapshot document: queryDocumentSnapshots){
                    Appointment appointment = document.toObject(Appointment.class);
    ...
    private void filterAppointmentsByGender(String filterKeyword) {
        mAppointments.clear();
        adapter1.notifyDataSetChanged();
        db.collection("patients").whereEqualTo("gender", filterKeyword).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                for(QueryDocumentSnapshot document: queryDocumentSnapshots){
                    Appointment appointment = document.toObject(Appointment.class);
    ...
    private void showAllAppointments() {
        db.collection("patients").get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            ...
            public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                for(QueryDocumentSnapshot document: queryDocumentSnapshots){
                    Appointment appointment = document.toObject(Appointment.class);
                    String uniqueId = db.collection("patients").document().getId();
